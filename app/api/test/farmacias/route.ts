...
export async function GET(request: Request) {
  try {
    ...
    console.log("🔍 Test Farmacias - Término:", query)

    // Primero obtener una muestra de datos para ver la estructura
    const { data: muestra, error: errorMuestra } = await supabase.from("Red de Farmacias").select("*").limit(5)

    if (errorMuestra) {
      console.error("Error obteniendo muestra:", errorMuestra)
    }

    console.log("📊 Muestra de Red de Farmacias:", muestra)

    // Ahora hacer la búsqueda
    const { data, error } = await supabase.from("Red de Farmacias").select("*").limit(500)
    ...
    console.log("📈 Total farmacias:", data?.length)
    ...
    if (query.length >= 2) {
      resultados =
        data?.filter((farm) => {
          const nombre = farm.Nombre || farm.nombre || ""
